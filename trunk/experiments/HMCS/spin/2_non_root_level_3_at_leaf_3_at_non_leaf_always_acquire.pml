...
bool LeftEdgeSwapped=false;
bool okToStartLeftEdgeRelease= false;
bool statusRoundOneA = true;
bool statusRoundOneB = true;
bool RightEdgeSwapped = false;
...
:: statusRoundOneA == true -> 
	...
	run WorkLevel2MinimalLeftEdge();
	LeftEdgeSwapped==true;
        SWAP(L2,pred_Acq_L2, myL2Id);
	assert(L2 == myL2Id); 
	run WorkLevel2MinimalRightEdge();
	RightEdgeSwapped==true;
...
/* arrange such that we are in the middle and aborted */
if
:: statusRoundOneB == true -> statusRoundOneB = false; assert(MY_L2_STATUS == ABORTED); okToStartLeftEdgeRelease = true;
:: else -> skip;
fi
...
inline AcquireL2MinimalLeftEdge(abortedLevel_Acq_L2_MLE)
{
    byte prevStatus_Acq_L2_MLE;
    byte pred_Acq_L2_MLE;
    SWAP(MY_L2_STATUS, prevStatus_Acq_L2_MLE, WAIT);

atomic{
    if
        :: prevStatus_Acq_L2_MLE == READY_TO_USE -> MY_L2_NEXT = NONE;
        :: else -> assert(0);
    fi
...
atomic{
    SWAP(L2,pred_Acq_L2_MLE, myL2Id);
    LeftEdgeSwapped=true;
};

    if
        :: atomic{ pred_Acq_L2_MLE == NONE ->
            MY_L2_STATUS = COHORT_START; };
            AcquireL3(abortedLevel_Acq_L2_MLE);
        :: else -> assert(0);
     fi
}
...
d_step{
  ...
  byte abortedLevel_AW_L2 = NONE;
  AcquireL2MinimalLeftEdge(abortedLevel_AW_L2);
  ...
  :: d_step{ abortedLevel_AW_L2==NONE -> 
      ...
      atomic{ assert(inCS == 1); 
      inCS--;
      /* wait so that nodes are in expected order before starting to release */ 
      okToStartLeftEdgeRelease == true; 
      }; ReleaseL2();
...
inline AcquireL2MinimalRightEdge(abortedLevel_Acq_L2_MRE)
{
    byte prevStatus_Acq_L2_MRE;
    byte pred_Acq_L2_MRE;
    byte predNext_Acq_L2_MRE;
    byte tmpStatus_Acq_L2_MRE;
    SWAP(MY_L2_STATUS, prevStatus_Acq_L2_MRE, WAIT);

atomic{
    if
        :: prevStatus_Acq_L2_MRE == READY_TO_USE -> MY_L2_NEXT = NONE;
        :: else -> assert(0);
    fi
...
USE_QNODE_L2: skip;
atomic{
    SWAP(L2,pred_Acq_L2_MRE, myL2Id);
/* let left edge go */
RightEdgeSwapped=true;
}

    if
        :: atomic{ pred_Acq_L2_MRE == NONE -> assert(0)};
        :: else ->
            /* Avoid unbounded wait for I->next */
            SWAP(L2_NEXT(pred_Acq_L2_MRE), predNext_Acq_L2_MRE, myL2Id);
            if
                :: d_step { predNext_Acq_L2_MRE == CANT_WAIT_FOR_NEXT ->
                    L2_STATUS(pred_Acq_L2_MRE)= READY_TO_USE;};
                    MY_L2_STATUS = COHORT_START;
                    AcquireL3(abortedLevel_Acq_L2_MRE);
                :: atomic { else ->
            SWAP(MY_L2_STATUS, prevStatus_Acq_L2_MRE , ABORTED); };
            atomic {
                if
                :: prevStatus_Acq_L2_MRE < ACQUIRE_PARENT -> MY_L2_STATUS  = prevStatus_Acq_L2_MRE ; goto DONE_ACQUIRE_L2_MRE;
                :: prevStatus_Acq_L2_MRE == ACQUIRE_PARENT -> MY_L2_STATUS  = COHORT_START ;
                :: else -> abortedLevel_Acq_L2_MRE = 1 /* L2 */; goto DONE_ACQUIRE_L2_MRE;
                fi
            };
            AcquireL3(abortedLevel_Acq_L2_MRE); 
     fi
    fi
    DONE_ACQUIRE_L2_MRE: skip;
}
...
d_step{
  ...
  byte abortedLevel_AW_L2 = NONE;
  AcquireL2MinimalRightEdge(abortedLevel_AW_L2);

...
static const char *pan_par[] = {	/* generates pan.p */
	...
	"	q += (256+2) * sizeof(AlignedULong_t); /* preserve contents of ncomps */",
	"#else",
	...
	"	assert(who_am_i == 0);",
	"",
	"	shared_mem_id = shmget(key, n, 0600 | IPC_CREAT | IPC_EXCL |SHM_HUGETLB|SHM_NORESERVE);	/* create */",
	"	if (shared_mem_id == -1)",
	...
	"		}",
	"#endif",
	"		temp_id = shmget(key, n, 0600|SHM_HUGETLB|SHM_NORESERVE);		/* check for stale copy */",
	"		if (temp_id != -1)",
	...
	"		}	}",
	"",
	"		temp_id = shmget(key, n, 0600 | IPC_CREAT | IPC_EXCL|SHM_HUGETLB|SHM_NORESERVE);	/* create new */",
	"		if (temp_id == -1)",
	"		{	n -= delta;",
	"			break;",
	"		} else",

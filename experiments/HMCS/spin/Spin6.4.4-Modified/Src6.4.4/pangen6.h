...
"        typedef union AlignedUInt{volatile unsigned int  ui;char pad[CACHE_LINE_SZ];} AlignedUInt_t;",
"#endif",
	"",
...
"#endif", /* WIN32 || WIN64 */
	...
	"",
	"volatile AlignedInt_t *prfree;",	/* [NCORE] */
	"volatile AlignedInt_t *prfull;",	/* [NCORE] */
	"volatile AlignedInt_t *prcnt;",		/* [NCORE] */
	"volatile AlignedInt_t *prmax;",		/* [NCORE] */
	"",
	"volatile AlignedInt_t /* int */ *sh_lock __attribute__((aligned(CACHE_LINE_SZ)));	/* to detect when processes crash */",
	"volatile AlignedDouble_t /* double*/ *is_alive __attribute__((aligned(CACHE_LINE_SZ)));	/* to detect when processes crash */",
	"volatile AlignedInt_t /* int */  *grfree __attribute__((aligned(CACHE_LINE_SZ)));	/* access to shared global q */",
	"volatile AlignedInt_t /* int */     *grfull __attribute__((aligned(CACHE_LINE_SZ)));	/* access to shared global q */",
	"volatile AlignedInt_t /* int */     *grcnt __attribute__((aligned(CACHE_LINE_SZ)));	/* access to shared global q */",
	"volatile AlignedInt_t /* int */     *grmax __attribute__((aligned(CACHE_LINE_SZ)));	/* access to shared global q */",
	"volatile AlignedDouble_t /* double */  *gr_readmiss __attribute__((aligned(CACHE_LINE_SZ)));",
	"volatile AlignedDouble_t /* double */  *gr_writemiss __attribute__((aligned(CACHE_LINE_SZ)));",
	...
	"	{	if (search_terminated->ui != 0)",
	"		{	if (verbose)",
	"			{	printf(\"cpu%%d: termination initiated (%%d)\\n\",",
	"					core_id, (int) search_terminated->ui);",
	"			}",
	"		} else",
	"		{	if (verbose)",
	"			{	printf(\"cpu%%d: initiated termination\\n\", core_id);",
	"			}",
	"			search_terminated->ui |= 8;	/* sudden_stop */",
	"		}",
	"		if (core_id == 0)",
	"		{	if (((search_terminated->ui) & 4)	/* uerror in one of the cpus */",
	"			&& !((search_terminated->ui) & (8|32|128|256))) /* abnormal stop */",
	...
	"}",
	"",
	"#define iam_alive()	is_alive[core_id].d++",	/* for crash detection */
	"",
	...
	"",
	"	if (search_terminated == NULL",
	"	|| search_terminated->ui != 0)",
	"	{",
	"		if (!(search_terminated->ui & (8|32|128|256)))",
	"		{	if (count++ < 100*NCORE)",
	"			{	return 0;",
	"		}	}",
	"		return 1;",
	...
	"	if (last_value == is_alive[(core_id + NCORE - 1) %% NCORE].d)",
	"	{	if (count++ >= 100)	/* to avoid unnecessary checks */",
	"		{	return 1;",
	"		}",
	"		return 0;",
	"	}",
	"	last_value = is_alive[(core_id + NCORE - 1) %% NCORE].d;",
	"	count = 0;",
	"	crash_reset();",
	"	return 0;",
	"}",
	...
	"		if (core_id == 0 && (gr_readmiss->d >= 1.0 || gr_readmiss->d >= 1.0 || grcnt->i != 0))",
	"		printf(\"cpu0: gq [readmiss: %%g, writemiss: %%g cnt %%d]\\n\", gr_readmiss->d, gr_writemiss->d, grcnt->i);",
	"#endif",
	"	}",
	...
	"	volatile struct Stack_Tree *prv; /* backward link towards root */",
        ...
        "       char xxxx[CACHE_LINE_SZ];",
        "       };",
	"} __attribute__((aligned(CACHE_LINE_SZ))) Stack_Tree;",
	"",
	"H_el *grab_shared(int);",
	"volatile Stack_Tree **stack_last __attribute__((aligned(CACHE_LINE_SZ))); /* in shared memory */",
	...
	"			shmid[m] = shmget(key[m], (size_t) qsize, 0600|SHM_HUGETLB|SHM_NORESERVE);",
	"			if (shmid[m] != -1)	/* yes there is one; remove it */",
	"			{	printf(\"cpu0: removing stale q%%d, status: %%d\\n\",",
	"					m, shmctl(shmid[m], IPC_RMID, NULL));",
	"			}",
	"			shmid[m] = shmget(key[m], (size_t) qsize, 0600|IPC_CREAT|IPC_EXCL|SHM_HUGETLB|SHM_NORESERVE);",
	"			memcnt += qsize;",
	"		} else			/* workers attach */",
	"		{	shmid[m] = shmget(key[m], (size_t) qsize, 0600|SHM_HUGETLB|SHM_NORESERVE);",
	...
	"",
	"	if (core_id == 0)	/* root */",
	"	{	shmid_S = shmget(key, n, 0600|SHM_HUGETLB|SHM_NORESERVE);",
	"		if (shmid_S != -1)",
	"		{	printf(\"cpu0: removing stale segment, status: %%d\\n\",",
	"				(int) shmctl(shmid_S, IPC_RMID, NULL));",
	"		}",
	"		shmid_S = shmget(key, n, 0600 | IPC_CREAT | IPC_EXCL |SHM_HUGETLB|SHM_NORESERVE);",
	"		memcnt += (double) n;",
	"	} else			/* worker */",
	"	{	shmid_S = shmget(key, n, 0600 |SHM_HUGETLB|SHM_NORESERVE);",
	"	}",
	...
	"",
	"	if (core_id == 0)",
	"	{	shmid_M = shmget(key, n, 0600 | SHM_HUGETLB|SHM_NORESERVE);",
	"		if (shmid_M != -1)",
	"		{	printf(\"cpu0: removing stale memory segment %%d, status: %%d\\n\",",
	"				cnt-3, shmctl(shmid_M, IPC_RMID, NULL));",
	"		}",
	"		shmid_M = shmget(key, n, 0600 | IPC_CREAT | IPC_EXCL | SHM_HUGETLB|SHM_NORESERVE);",
...
"#ifdef SEP_STATE",
	...
	"		exit(1);",
	"	}",
	"	search_terminated = (volatile AlignedUInt_t *) x; /* comes first */",
	"	x += sizeof(AlignedUInt_t); /* maintain alignment */",
	"",
	"	is_alive   = (volatile AlignedDouble_t *) x;",
	"	x += NCORE * sizeof(AlignedDouble_t);",
	"",
	"	sh_lock   = (volatile AlignedInt_t *) x;",
	"	x += CS_NR * sizeof(AlignedInt_t);", /* allow 1 word per entry */
	"",
	"	grfree    = (volatile AlignedInt_t *) x;",
	"	x += sizeof(AlignedInt_t);",
	"	grfull    = (volatile AlignedInt_t *) x;",
	"	x += sizeof(AlignedInt_t);",
	"	grcnt    = (volatile AlignedInt_t *) x;",
	"	x += sizeof(AlignedInt_t);",
	"	grmax    = (volatile AlignedInt_t *) x;",
	"	x += sizeof(AlignedInt_t);",
	"	prfree = (volatile AlignedInt_t *) x;",
	"	x += NCORE * sizeof(AlignedInt_t);",
	"	prfull = (volatile AlignedInt_t *) x;",
	"	x += NCORE * sizeof(AlignedInt_t);",
	"	prcnt = (volatile AlignedInt_t *) x;",
	"	x += NCORE * sizeof(AlignedInt_t);",
	"	prmax = (volatile AlignedInt_t *) x;",
	"	x += NCORE * sizeof(AlignedInt_t);",
	"	gr_readmiss    = (volatile AlignedDouble_t *) x;",
	"	x += sizeof(AlignedDouble_t);",
	"	gr_writemiss    = (volatile AlignedDouble_t *) x;",
	"	x += sizeof(AlignedDouble_t);",
	"",
	"	#ifdef FULL_TRAIL",
...
"#else",
	...
	"			x += sizeof(AlignedDouble_t);",
	" #ifdef FULL_TRAIL",
	"			stack_last = (volatile Stack_Tree_t **) x;",
	"			x += NCORE * sizeof(Stack_Tree *);",
	" #endif",
	"			if (((long)x)&(CACHE_LINE_SZ-1)) /* CacheLine word alignment */",
	"			{	x += CACHE_LINE_SZ-(((long)x)&(CACHE_LINE_SZ-1));",
	"			}",
	"",
	"			#ifdef COLLAPSE",
	"			ncomps = (AlignedULong_t *) x;",
	"			x += (256+2) * sizeof(AlignedULong_t);",
	"			#endif",
	"		}",
	"",
...
"#endif", /* !SEP_STATE */
	...
	"	}",
	"	if (search_terminated != NULL)",
	"	{	search_terminated->ui |= 16; /* cleanup_shm */",
	"	}",
	"",
	"	for (m = 0; m < NR_QS; m++)",
	...
	"	#endif",
	"		if (grcnt->i > 0)	/* accessed outside lock, but safe even if wrong */",
	"		{	enter_critical(GQ_RD);	/* gq - read access */",
	"			if (grcnt->i > 0)		/* could have changed */",
	"			{	f = &m_workq[NCORE][grfull->i];	/* global q */",
	"				if (f->m_vsize == 0)",
	"				{	/* writer is still filling the slot */",
	"					gr_writemiss->d++;",
	"					f = &m_workq[n][prfull[n].i]; /* reset */",
	"				} else",
	"				{	grfull->i = (grfull->i+1) %% (GN_FRAMES);",
	...
	"	prfull[n].i = (prfull[n].i + 1) %% (LN_FRAMES);",
	"	enter_critical(QLOCK(n));",
	"		prcnt[n].i--; /* lock out increments */",
	...
	"	}	}	}",
	"	if (n != NCORE)",
	"	{	prfree[n].i = (prfree[n].i + 1) %% (LN_FRAMES);",
	"		enter_critical(QLOCK(n));",
	"			prcnt[n].i++; /* lock out decrements */",
	"			if (prmax[n].i < prcnt[n].i)",
	"			{	prmax[n].i = prcnt[n].i;",
	"			}",
	"		leave_critical(QLOCK(n));",
	"	}",
	...
	"",
	"	gq_tries++;",
	"	if (grcnt->i < GN_FRAMES) /* there seems to be room */",
	"	{	enter_critical(GQ_WR);	/* gq write access */",
	"		if (grcnt->i < GN_FRAMES)",
	"		{	if (m_workq[NCORE][grfree->i].m_vsize != 0)",
	"			{	/* can happen if reader is slow emptying slot */",
	"				gr_readmiss->d++;",
	"				goto out; /* dont wait: release lock and return */",
	"			}",
	"			lrfree = grfree->i;	/* Get_Free_Frame use lrfree in this mode */",
	"			grfree->i = (grfree->i + 1) %% GN_FRAMES;",	/* next process looks at next slot */
	"			grcnt->i = grcnt->i + 1;	/* count nr of slots filled -- no additional lock needed */",
	"			if (grmax->i < grcnt->i) grmax->i = grcnt->i;",
	...
	"}",
	"",
	"#define TargetQ_Full(n)	(m_workq[n][prfree[n].i].m_vsize != 0)", /* no free slot */
	"#define TargetQ_NotFull(n)	(m_workq[n][prfree[n].i].m_vsize == 0)", /* avoiding prcnt */
	"",
	"int",
	"AllQueuesEmpty(void)",
	"{	int q;",
	...
	"#endif",
	"	for (q = 0; q < NCORE; q++)",
	"	{	if (prcnt[q].i != 0)", /* not locked, ok if race */
	"		{	return 0;",
	"	}	}",
	"	return 1;",
	"}",
	...
	"					core_id, search_terminated?search_terminated->ui:-1);",
	"				sudden_stop(\"\");",
	"				pan_exit(1);",
	"			}",
	"#ifdef TESTING",
	...
	"					target_q, nstates_get + 1, prfree[target_q].i-1);",
	"				}",
	...
	"					q, f->m_vsize, nstates_put + 1, prfull[q].i-1);",
	"				snapshot();",
	"			}",
	"			remember = f->m_vsize;",
	...
	"				&&  grfree->i + 1 == remember)	/* no action on global queue meanwhile */",
	...
	"					target_q, remember, grfree->i + 1, prfree[target_q].i-1);",
	"				if (!f) { Uerror(\"Fatal4: no free slot\"); }",
	"",
	"				if (grfree->i + 1 == remember)	/* no action on global queue */",
	...
	"{",
	"	if (search_terminated != NULL)",
	"	{	search_terminated->ui |= 32;	/* give_up */",
	"	}",
	"	if (!writing_trail)",
	"	{	was_interrupted = 1;",
	"		snapshot();",
	...
	"{",
	"	if (search_terminated == NULL",
	"	||  search_terminated->ui != 0)	/* not a full crash check */",
	"	{	pan_exit(0);",
	"	}",
	...
	"	#ifdef GENEROUS",
	"		if (prcnt[q].i < LN_FRAMES)", /* not the best strategy */
	"	#else",
	"		if (TargetQ_NotFull(q)",
	"		&& (dfs_phase2 == 0 || prcnt[core_id].i > 0))", /* not locked, ok if race */
	"	#endif",

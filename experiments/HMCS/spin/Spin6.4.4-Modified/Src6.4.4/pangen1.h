...
static const char *Code2a[] = { /* the tail of procedure run() */
	...
	"		#ifdef BFS_SEP_HASH",
	"		ncomps = (ulong *) emalloc((ulong)((256+2) * sizeof(AlignedULong_t)));",
	"		#else",
	"		ncomps = (ulong *) sh_pre_malloc((ulong)((256+2) * sizeof(AlignedULong_t)));",
	"		#endif",
	"	#endif",
	"#endif",
...
"#endif",
	...
	"volatile AlignedUInt_t *search_terminated; /* to signal early termination */",
	/*
	...
	"	{	search_terminated->ui |= 1;	/* pan_exit */",
	"	}",
		"#ifdef USE_DISK",
	"	{ void	dsk_stats(void);",
	"		dsk_stats();",
...
"#include <unistd.h>",
	...
	"#endif",
        "#ifdef USE_HUGE_PAGE",
	"	    tmp = (char *) mmap(0,n,PROT_READ|PROT_WRITE, MAP_PRIVATE|MAP_ANONYMOUS|MAP_HUGETLB|MAP_NORESERVE|MAP_POPULATE,-1,0);",
	"	if(tmp == MAP_FAILED)",
	"	    tmp = 0;",
        "#else",
	"	tmp = (char *) malloc(n);",
	...
	"",
        "#ifdef USE_HUGE_PAGE",
        "#define CHUNK ((100*VECTORSZ) > (1024*1024*2)? (100*VECTORSZ) : (1024*1024*2))",
        "#else",

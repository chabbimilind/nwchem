...
atomic{
    ...
    SWAP(L2, pred_Acq_L2_WL2M, myL2Id);
     ...
     assert(predStat_Acq_L2_WL2M != CANT_WAIT_FOR_NEXT);
/** Inform mid ***/
    ch1!true; 
}; // end atomic
              ...
              atomic {
                      if 
                      :: MY_L2_STATUS == UNLOCKED ->  gotLock_WL2M = true;
                      :: else -> MY_L2_STATUS = ABORTED;
                      fi;
               };

       if
       :: gotLock_WL2M -> ReleaseL2();
       :: else -> skip;
       fi;
...
inline DealWithRestOfLeveL2LeftEdge(me_DWR_L2_LE /* destroyed */ , prev_LE /* destroyed and used by caller*/ ){
    ...
    :: d_step{else ->
       ...
       if 
       :: atomic{retOld_L2_LE == false -> assert(0);};
       :: atomic{else -> 
		L2_STATUS(retOld_L2_LE) = READY_TO_USE;
		break;};
       fi
     od
}
...
atomic{
		...
		// let mid to go on
		if
		:: setup3 == false -> setup3 = true; ch3!true;
		:: else -> skip;
		fi;
}

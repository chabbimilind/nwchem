...
/*atomic{
    ...
    :: d_step{prevStatus_Acq_L2 == READY_TO_USE -> skip;};
    :: d_step{prevStatus_Acq_L2 == WAIT -> skip;};
    :: prevStatus_Acq_L2 == UNLOCKED ->
        ...
        if
        :: d_step{tmpStatus != READY_TO_USE -> abortedLevel_Acq_L2 = 1; /* L2 */}; goto DONE_ACQUIRE_L2;
        :: d_step{else -> skip; };
        fi
    :: else -> assert (false);
    fi
    ...
    START_SPIN_L2: skip;
                   ...
                   :: else -> 
		            SWAP(MY_L2_STATUS, prevStatus_Acq_L2 , ABORTED);
	                    if
                	    :: d_step {prevStatus_Acq_L2 == UNLOCKED -> MY_L2_STATUS = UNLOCKED; abortedLevel_Acq_L2 = NONE; };
                            :: d_step {else -> abortedLevel_Acq_L2 = 1;};
                    	    fi
                    fi
            fi
       ...
       :: d_step{
            ...
            ::d_step{retOld == false ->
              ...
              :: d_step{
               ...
               :: d_step{else -> L2_STATUS(me_DWR_L2) = READY_TO_USE;}; break;
               fi    
            :: else -> break;
            fi
       :: d_step{else -> L2_STATUS(me_DWR_L2) = READY_TO_USE;}; break;
...
inline ReleaseL2()
{
    ...
    :: else ->
       ...
       :: HAS_VALID_L2_SUCC(curNode_Rel_L2) -> 
          ...
          :: d_step{ else -> MY_L2_STATUS = READY_TO_USE;}; goto CLEANUP;
          fi 
       :: else -> 
...
inline AcquireL1(abortedLevel_Acq_L1)
{
  ...
  :: d_step {prevStatus_Acq_L1 == WAIT -> skip;};
  :: d_step {prevStatus_Acq_L1 == READY_TO_USE -> skip;};
  :: else ->
        /*while(I->status != READY_TO_USE); No unbounded wait*/
     CAS(MY_L1_STATUS, WAIT, ACQUIRE_PARENT, tmpStatus_Acq_L1);
     if
     :: d_step {tmpStatus_Acq_L1 != READY_TO_USE -> abortedLevel_Acq_L1 = 0 /* L1 */; }; 
...
START_SPIN_L1: skip;
           ...
           if
           :: d_step {prevStatus_Acq_L1 < ACQUIRE_PARENT -> MY_L1_STATUS  = prevStatus_Acq_L1 ; abortedLevel_Acq_L1 = NONE; }; 
           :: d_step {prevStatus_Acq_L1 == ACQUIRE_PARENT -> MY_L1_STATUS  = COHORT_START ;};  AcquireL2(abortedLevel_Acq_L1); 
           :: d_step {else -> abortedLevel_Acq_L1 = 0 /* L1 */; }; 
           fi
      fi
  fi
  ...
  :: d_step{else ->
     ...
     :: retOld_DWR_L1 == false ->
        ...
        :: d_step{retOld_DWR_L1 == false ->
           ...
           :: d_step{else -> L1_STATUS(me_DWR_L1) = READY_TO_USE;}; break;
           fi
        :: else -> break;
        fi
      :: d_step{else -> L1_STATUS(me_DWR_L1) = READY_TO_USE;}; break;
    ...
    :: else ->
       ...
       :: HAS_VALID_L1_SUCC(curNode_HHP_L1) ->
         SWAP(L1_STATUS(L1_NEXT(curNode_HHP_L1)), prevStatus_HHP_L1,  value_HHP_L1);
          ...
          :: d_step{else -> L1_STATUS(curNode_HHP_L1) = READY_TO_USE;}; break;
          fi
  ...
  do
    :: else ->
      ...
      :: HAS_VALID_L1_SUCC(curNode_HHA_L1) ->
         ...
         :: d_step{else -> L1_STATUS(curNode_HHA_L1) = READY_TO_USE;}; break;
         fi

...
#define MY_L1_NODE_ID(id) (id < MAX_L1_THREADS -> id : 0)

#define MY_L2_NODE_ID(id) (id < MAX_L1_THREADS -> 0 : (id < (MAX_L1_THREADS + MAX_L2_THREADS - 1) -> id - (MAX_L1_THREADS- 1) : 0) )

#define MY_L3_NODE_ID(id) (id <= (MAX_L1_THREADS + MAX_L2_THREADS - 1) -> 0 : id - 3)
...
/* arrange in this fashion: 1 2 0 */
atomic{
if
:: myL2Id==1 && L2 != NONE -> ; goto FINISHED; 
:: myL2Id==2 &&  L2 !=1 ->  goto FINISHED; 
:: myL2Id==0 && statusRoundOneA == true -> 
	if 
	:: L2 == 2 -> statusRoundOneA = false;
	:: else ->  goto FINISHED;
	fi;
:: else ->  skip;
fi;
...
START_SPIN_L1: skip;
           ...
           atomic{
             ...
             :: prevStatus_Acq_L1 == ACQUIRE_PARENT -> MY_L1_STATUS  = COHORT_START; /* check correctness */ inCSL1++;
...
proctype  WorkObserved(byte mypid) 
{ 
  bool acquired;
  byte numRounds = 2;
  byte myL1Id;
...
d_step{
  myL1Id = MY_L1_NODE_ID(mypid);
  myL2Id = MY_L2_NODE_ID(mypid);
};
  ...
  :: atomic{ else -> gDone++; break; };
  od
FINISHED:skip;
}

proctype  WorkLevel1(byte mypid) 
{ 
  bool acquired;
  byte myL1Id;
  byte myL2Id;
...
proctype  WorkLevel2(byte mypid) 
{ 
  bool acquired;
  byte myL1Id;
  byte myL2Id;
...
init {
  ...
  run WorkLevel2(3); 
  run WorkLevel2(4); 
  (_nr_pr==1); // wait to finish
  if 
  :: gDone == 5 ->  EnsureAllinRState(); assert(totalAcquisitions >= 1);
  :: else -> skip;
  fi;
} 

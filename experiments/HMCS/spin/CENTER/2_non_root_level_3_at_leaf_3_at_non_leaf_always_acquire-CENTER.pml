...
byte inCSL1;
byte done=0;
byte totalSuccessfulAcquisitions=0;
...
USE_QNODE_L2: skip;
/* arrange such that we are in the center */
atomic{
if
...
/* validate that only one thread is here */
        inCSL1++;
      };
/* validate that only one thread is here */
...
START_SPIN_L1: skip;
           ...
           atomic{
             ...
             :: prevStatus_Acq_L1 == ACQUIRE_PARENT -> MY_L1_STATUS  = COHORT_START;  /* validate that only one thread is here */ inCSL1++;
...
D_proctype  WorkObserved(byte mypid) 
{ 
  bool acquired;
  byte numRounds = 2;
  byte myL1Id;
...
D_proctype  WorkLevel1(byte mypid) 
{ 
  bool acquired;
  byte myL1Id;
  byte myL2Id;
...
//  myL2Id = MY_L2_NODE_ID(_pid-1);
  ...
  AcquireL1(abortedLevel_AW_L1);
  if
  :: abortedLevel_AW_L1==NONE ->  ReleaseL1();
...
D_proctype  WorkLevel2(byte mypid) 
{ 
  bool acquired;
  byte myL1Id;
  byte myL2Id;
...
/* arrange such that myL2id==1 is in the left and myL2id==0 is in the middle and myL2id==2 is in the right */
atomic{
//  myL1Id = MY_L1_NODE_ID(_pid-1);
//  myL2Id = MY_L2_NODE_ID(_pid-1);
  myL1Id = MY_L1_NODE_ID(mypid);
  myL2Id = MY_L2_NODE_ID(mypid);
        if
                ::myL2Id==2 -> end: (L2==0); /* till the center guy is 0 */
                ::else       -> skip;
        fi;
}
  ...
  AcquireL2(abortedLevel_AW_L2);
  if
  :: abortedLevel_AW_L2==NONE ->  ReleaseL2();
...
if
:: done == 5 ->  EnsureAllinRState(); assert(totalSuccessfulAcquisitions > 0);
:: else ->  skip;
fi;
} 

...
struct MCSAbortableLock{
  ...
  bool Acquire(int64_t T) {
      ...
      if (is_transient(temp)) {
          ...
          /* Store pred for future reference, if predecessor has not
              modified my prev pointer already. */
          (void) CAS(&I->prev, transient(nil), pred);
      }
      ...
      while (1) {
          ...
          } while (!BOOL_CAS(&I->next, succ, leaving_self));
      ...
      if (succ == nil) {
          ...
          if (BOOL_CAS(L, I, pred)) {
              ...
              temp = CAS(&pred->next, transient(nil), gone);
              if (temp != transient(nil)) {
                  ...
                  temp = cleaned(temp);
                  (void) CAS(&temp->prev, transient(nil), nil);
              }
          } else {
  ...
  serendipity:    // I got the lock after all;
      ...
      if (succ == nil) {
          ...
          if ((temp = CAS(&I->next, leaving_self, succ))
                  != leaving_self) {
  ...
  void Release(int64_t T) {
      ...
      while (1) {
          ...
          if (BOOL_CAS(&I->next, succ, granted(nil))) {
...
MCSAbortableLock * mcsAbortablelock;
MCSAbortableLock * LockInit(int tid, int maxThreads, int levels, int * participantsAtLevel){
#pragma omp barrier
#pragma omp single
	{
		mcsAbortablelock = new MCSAbortableLock();
	}
#pragma omp barrier
	return mcsAbortablelock;
}

#define UNDERLYING_LOCK MCSAbortableLock

...
struct PthreadLock{
    ...
    inline bool Acquire(struct timespec patience){
	//bool val = pthread_mutex_timedlock(&L, &patience) == 0;
	bool val = pthread_mutex_lock(&L) == 0;
	return val;
        //return pthread_mutex_reltimedlock_np(&L, &patience) == 0;
    }

    inline void Release(struct timespec patience){
	pthread_mutex_unlock(&L);
    }
};

PthreadLock* pthreadLock;
PthreadLock* LockInit(int tid, int maxThreads, int levels, int * participantsAtLevel){
#pragma omp barrier
#pragma omp single
    {
        pthreadLock = new PthreadLock();
    }
#pragma omp barrier
    return pthreadLock;
}

#define UNDERLYING_LOCK PthreadLock
...
if (hmcs->Acquire(waitThreshold) == true ) { \
mySuccessfulAcquisitions++;\
CriticalSection();\
hmcs->Release(waitThreshold);}\
...
#pragma omp parallel
    {
        ...
        if (waitThreshold > 999999999) {
		tspec.tv_sec = waitThreshold/(1000000000L);
		tspec.tv_nsec = waitThreshold%(1000000000L);
	} else {
		tspec.tv_sec = 0;
		tspec.tv_nsec = waitThreshold;
	}
        ...
        RUN_LOOP(warmupRounds, 10, tspec);

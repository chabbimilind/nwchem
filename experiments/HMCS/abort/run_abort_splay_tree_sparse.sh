...
do
for mult in 576 288 144 72 36 18 9 6 3 2 1
do
...
#  numactl --localalloc ./MCS_NB_abort $mult $timeout $nIter $waitThreshold $nThreads $wokingThreads 1 $nThreads $level4TH
#  numactl --localalloc ./CLH_NB_abort $mult $timeout $nIter $waitThreshold $nThreads $wokingThreads 1 $nThreads $level4TH
timeout 60  numactl --localalloc ./A_BO_CLH_NB_abort $mult $timeout $nIter $waitThreshold $nThreads $wokingThreads 2 36 $level2TH $nThreads $level4TH
done
done

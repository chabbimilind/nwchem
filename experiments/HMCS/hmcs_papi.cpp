...
void handle_error(int arg, int thread) {
	printf("Error! %d by thread %d\n", arg, thread);
	exit(1);
}

unsigned long get_thread_num() {
...
#define NUM_EVENTS 1
//char* eventNames[] = {"L3_CACHE_MISSES:ANY_READ"};
char* eventNames[] = {"L3_CACHE_MISSES:ANY_READ"};
...
int main(int argc, char *argv[]){
    ...
    for (int i = 0; i < levels; i++) {
        participantsAtLevel[i] = atoi(argv[5 + i]);
    }
    ...
    if (retval != PAPI_OK) handle_error(retval, 0);
    
    // Add native events to the eventset
...
#pragma omp barrier
	...
	for (int i = 0; i < NUM_EVENTS; i++) {
        printf("\n %s = %e",eventNames[i], (double)values[i]);
    }
